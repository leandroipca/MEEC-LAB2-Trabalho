...
LBPHFace = cv2.face.LBPHFaceRecognizer_create() # Create LBPH FACE RECOGNISER
...
def getImageWithID (path):
    ...
    for imagePath in imagePaths:
        ...
        faceImage = faceImage.resize((220,220))         # resize the image so the EIGEN recogniser can be trained
        faceNP = np.array(faceImage, 'uint8')           # convert the image to Numpy array
        ID = int(os.path.split(imagePath)[1].split('_')[0].replace("G", "")) # Retreave the ID of the array
...
EigenFace.save('Recogniser/trainingDataEigen.xml')
print('FILE SAVED..')
